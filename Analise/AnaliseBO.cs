...
namespace Analise.BLL
{
    public class AnaliseBO
    {
        ...
        private List<CheckListGrupoDTO> MontagemCheckListNova()
        {
            ...
            ficha.ListaItens.Add(new CheckListItemDTO("A ficha cadastral devidamente preenchida", EnumResultadoAnalise.EmAnalise));
        ...
        public List<ArquivoDTL> ListarArquivos(int idAnalise)
        {
            return new AnaliseDAO().ListarArquivos(idAnalise);
        }

        public string VerificarMinutaContrato(int idAnalise)
        {
            string retorno = "";
            int tipoMinuta = new AnaliseDAO().VerificarMinutaContrato(idAnalise);

            switch (tipoMinuta)
            {
                case 0:
                    retorno = "Aguardando a assinatura da minuta";
                    break;

                case 1:
                    retorno = "Minuta assinada Digitalmente";
                    break;

                default:
                    retorno = "Minuta Assinada e Anexada";
                    break;
            }
